...
class GerenciadorPlanilha:
    ...
    def adiciona_grafico_linha(self, celula: str, comprimento: float, altura: float, titulo: str,
        ...
        grafico.x_axis.title.font = Font(size=14, bold=True)
        grafico.y_axis.title = titulo_eixo_y
        grafico.y_axis.title.font = Font(size=14, bold=True)

        grafico.add_data(referencia_eixo_x, titles_from_data=True)
